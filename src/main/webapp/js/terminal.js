...
function printToTerminal() {
  type("Initiating systems...")
    .then("Connecting to Target and Sentry systems... ")
    .then("Engaging all Targets... OK")
    .wait(500)
    .then("Checking Ammunition... OK")
    .wait(500)
...
function createDots() {
  for (var i = 0; i < 2240; i++) {
    // Create a dot
    var dot = new Dot(randomNumber(0, stageWidth), randomNumber(0, stageHeight),
      colors[randomNumber(1, colors.length)], .3);
...
function drawNumber(num) {
  ...
  for (var i = imageData.length; i >= 0; i -= 4) {
    ...
    if (imageData[i] !== 0) {
      ...
      if ((x && x % (circleRadius * 2 + 3) == 0) && (y && y % (circleRadius * 2 +
          3) == 0)) {
